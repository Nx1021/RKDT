...
if __name__ == "__main__":
    ...
    for k, v in weights.items():
        if k != 7 and k != 8 and k != 9 and k!= 10:
            continue
        setup_paras["sub_data_dir"] = "linemod_o/"
        setup_paras["ldt_branches"] = {k: "linemod_mix_new/{}".format(v)}
        setup_paras["batch_size"] = batch_size
        # setup_paras["dataset_format"] = VocFormat_6dPosture

        predictor = setup("predict", 
                        detection_base_weight=f"{WEIGHTS_DIR}/linemod_mix_new/{str(k).rjust(6, '0')}_best.pt" ,
                        **setup_paras)
